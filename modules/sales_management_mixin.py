...
class SalesManagementMixin:
    ...
    def _forecast_build_inventory_source_lines(self, oid, is_on_market, plan_items, brief):
        """仅含替代发货方案中的替代 SKU（跨方案配比累加），不含下单 SKU 本体。"""
    ...
    def _forecast_order_row_inventory_aggregate(
        ...
        agg = self._forecast_inventory_tier_sum_assembled(inv_by_op, pairs)
            ...
            if forecast_mode == 'order':
                ...
                if pairs_vid:
                    ...
                    for oid, qp in pairs_vid:
                        ...
                        ai = self._forecast_inventory_tier_sum_assembled(inv_by_op, pairs_line)
                        per_link.append({
    ...
    def _container_add_days_ymd(self, ymd, days):
        base = self._forecast_format_date_only(ymd)
        if not base:
            return self._forecast_format_date_only(datetime.now())
        try:
    ...
    def _container_month_key_from_ymd(self, ymd):
        base = self._forecast_format_date_only(ymd)
        if not base or len(base) < 7:
            return None
        return f'{base[:7]}-01'

    def _container_forecast_demand(self, row, month_key):
        fc = (row.get('forecasts') or {}).get(month_key) or {}
        return max(0, int(self._parse_int(fc.get('value_qty')) or 0))

    def _container_total_overseas_qty(self, detail):
        by = (detail or {}).get('overseas_by_region') or {}
        total = 0
        for _k, v in by.items():
            total += max(0, int(float(v or 0)))
        return total

    def _container_compute_full_surplus_series(self, row, months, lead_time_region, lead_times, mtd_ctx=None):
        """与销量预测页 computeOrderSkuSurplusSeries 全链路盈余逻辑对齐（简化返回）。"""
        detail = row.get('inventory_surplus_detail') or {}
        region_name = str(lead_time_region or '').strip()
        lt = (lead_times or {}).get(region_name) or {}
        stock_days = max(0, int(self._parse_int(lt.get('stock_days')) or 0))
        wip_days = max(0, int(self._parse_int(lt.get('wip_days')) or 0))
        today = self._forecast_format_date_only(datetime.now())

        overseas_init = self._container_total_overseas_qty(detail)
        transit_batches = list(detail.get('transit_batches') or [])

        transit_arrivals = {}
        for b in transit_batches:
            mk = self._container_month_key_from_ymd(b.get('expected_listed_date') or today)
            q = max(0, int(float(b.get('qty') or 0)))
            if mk and q > 0:
                transit_arrivals[mk] = int(transit_arrivals.get(mk) or 0) + q

        full_arrivals = dict(transit_arrivals)
        factory_stock = max(0, int(float(detail.get('factory_stock_qty') or 0)))
        factory_stock_month = ''
        if factory_stock > 0 and region_name:
            factory_stock_month = self._container_month_key_from_ymd(
                self._container_add_days_ymd(today, stock_days)
            )
            if factory_stock_month:
                full_arrivals[factory_stock_month] = int(full_arrivals.get(factory_stock_month) or 0) + factory_stock

        wip_arrivals = {}
        for w in (detail.get('wip_batches') or []):
            q = max(0, int(float(w.get('qty') or 0)))
            if not q or not region_name:
                continue
            comp = self._forecast_format_date_only(w.get('expected_completion_date') or today) or today
            mk = self._container_month_key_from_ymd(self._container_add_days_ymd(comp, wip_days))
            if not mk:
                continue
            wip_arrivals[mk] = int(wip_arrivals.get(mk) or 0) + q
            full_arrivals[mk] = int(full_arrivals.get(mk) or 0) + q

        sorted_months = sorted([str(m) for m in (months or [])], key=lambda x: str(x))
        month_set = set(sorted_months)
        cur_key = ''
        in_hist = False
        if mtd_ctx:
            cur_key = str(mtd_ctx.get('current_month_key') or '')
            in_hist = bool(mtd_ctx.get('in_history_range'))

        transit_bal = overseas_init
        full_bal = overseas_init
        full_surplus = {}
        month_detail = {}
        first_shortage_month = None

        for idx, mk in enumerate(sorted_months):
            gross = self._container_forecast_demand(row, mk)
            sold_mtd = 0
            if in_hist and cur_key and str(mk) == cur_key:
                hist = (row.get('history') or {}).get(mk) or {}
                sold_mtd = max(0, int(self._parse_int(hist.get('sales_qty')) or 0))

            tr_in = int(transit_arrivals.get(mk) or 0)
            full_in = int(full_arrivals.get(mk) or 0)
            factory_in = max(0, full_in - tr_in)

            transit_before = transit_bal
            full_before = full_bal
            if sold_mtd > 0:
                transit_bal += sold_mtd
                full_bal += sold_mtd
            transit_bal += tr_in
            full_bal += full_in

            tr_snap = transit_bal - gross
            full_snap = full_bal - gross
            full_surplus[mk] = full_snap
            if first_shortage_month is None and full_snap < 0:
                first_shortage_month = mk

            month_detail[mk] = {
                'gross_demand': gross,
                'sold_mtd': sold_mtd,
                'transit_arrival': tr_in,
                'factory_arrival': factory_in,
                'full_arrival': full_in,
                'full_balance_before': full_before,
                'full_surplus': full_snap,
            }
            transit_bal = tr_snap
            full_bal = full_snap
        ...
        return {
            ...
            'first_shortage_month': first_shortage_month,
            'overseas_init': overseas_init,
            'lead_time_region': region_name,
        }

    def _container_suggest_ship_qty(self, row, months, surplus_bundle, target_cover_months):
        full_surplus = surplus_bundle.get('full_surplus') or {}
        if not months:
            return 0, {}
        sorted_months = sorted([str(m) for m in months], key=lambda x: str(x))
        min_surplus = min([int(full_surplus.get(m) or 0) for m in sorted_months], default=0)
        last_mk = sorted_months[-1]
        end_surplus = int(full_surplus.get(last_mk) or 0)

        demands = [self._container_forecast_demand(row, m) for m in sorted_months]
        avg_demand = (sum(demands) / float(len(demands))) if demands else 0.0
        cover = max(0.0, float(target_cover_months or 0))
        desired_end = cover * avg_demand if cover > 0 else 0.0
        shortfall_cover = max(0.0, desired_end - float(end_surplus))
        deficit = max(0, -int(min_surplus))
        raw = int(max(deficit, shortfall_cover, 0))

        meta = {
            'min_full_surplus': min_surplus,
            'end_full_surplus': end_surplus,
            'avg_monthly_demand': round(avg_demand, 2),
            'first_shortage_month': surplus_bundle.get('first_shortage_month'),
        }
        return raw, meta

    def _container_load_pack_meta_by_order(self, conn, order_product_ids):
        ids = sorted({int(x) for x in (order_product_ids or []) if self._parse_int(x)})
        out = {}
        if not ids:
            return out
        ...
        with conn.cursor() as cur:
            ...
            for rr in cur.fetchall() or []:
                ...
                if not oid:
                    continue
                carton = self._parse_int(rr.get('carton_qty'))
                if carton is not None and int(carton) <= 0:
                    carton = None
                vol = None
                calc_carton = self._calc_carton_qty_by_40hq(
                    rr.get('package_length_in'),
                    rr.get('package_width_in'),
                    rr.get('package_height_in'),
                )
                if calc_carton and int(calc_carton) > 0:
                    vol = 69.0 / float(calc_carton)
                elif carton and int(carton) > 0:
                    vol = 69.0 / float(carton)
                out[oid] = {
                    'sku': (rr.get('sku') or '').strip(),
                    'carton_qty': int(carton) if carton else None,
                    'unit_volume_m3': vol,
                    'carton_qty_calc': int(calc_carton) if calc_carton else None,
                }
            cur.execute(
                f"""
                ...
                WHERE opl.order_product_id IN ({ph})
                ORDER BY opl.order_product_id ASC, lf.factory_name ASC, opl.factory_id ASC
                """,
                tpl,
            )
            for rr in cur.fetchall() or []:
                ...
                if not oid or oid not in out:
                    continue
                rec = out[oid]
                if rec.get('factory_id'):
                    continue
                fid = self._parse_int(rr.get('factory_id'))
                if fid:
                    rec['factory_id'] = fid
                    rec['factory_name'] = (rr.get('factory_name') or '').strip() or f'#{fid}'
        for oid, rec in out.items():
            rec.setdefault('factory_id', 0)
            rec.setdefault('factory_name', '未关联工厂')
        return out

    def _container_round_up_cartons(self, qty, carton_qty):
        q = max(0, int(qty) or 0)
        if q <= 0:
            return 0
        c = max(1, int(carton_qty) or 1)
        return ((q + c - 1) // c) * c

    def _container_pack_lines_by_factory(self, lines, container_volume_m3, min_fill_ratio=0.75):
        vol_cap = max(1.0, float(container_volume_m3 or 69.0))
        min_fill = min(0.99, max(0.1, float(min_fill_ratio or 0.75)))

        by_factory = {}
        for ln in lines:
            fid = int(ln.get('factory_id') or 0)
            by_factory.setdefault(fid, []).append(ln)

        containers = []
        seq = 0
        for fid in sorted(by_factory.keys(), key=lambda x: (x == 0, x)):
            factory_lines = by_factory[fid]
            factory_name = (factory_lines[0].get('factory_name') if factory_lines else '') or '未关联工厂'
            factory_lines.sort(
                key=lambda x: (
                    0 if x.get('first_shortage_month') else 1,
                    str(x.get('first_shortage_month') or '9999'),
                    -int(x.get('suggested_qty') or 0),
                )
            )
            ...
            def _flush():
                nonlocal seq, current_lines, used_vol
                if not current_lines:
                    return
                seq += 1
                total_units = sum(int(x.get('pack_qty') or 0) for x in current_lines)
                containers.append({
                    'container_no': seq,
                    'factory_id': fid,
                    ...
                    'capacity_m3': vol_cap,
                    'fill_ratio': round(used_vol / vol_cap, 4) if vol_cap > 0 else 0,
                    'total_units': total_units,
                })
                current_lines = []
                used_vol = 0.0

            for ln in factory_lines:
                remain = int(ln.get('suggested_qty') or 0)
                if remain <= 0:
                    continue
                carton = max(1, int(ln.get('carton_qty') or 1))
                unit_vol = float(ln.get('unit_volume_m3') or (vol_cap / float(carton)))
                if unit_vol <= 0:
                    unit_vol = vol_cap / float(carton)

                while remain > 0:
                    space = vol_cap - used_vol
                    max_units = int(space / unit_vol) if unit_vol > 0 else 0
                    if max_units < carton:
                        if current_lines and (used_vol / vol_cap) >= min_fill:
                            _flush()
                            continue
                        if current_lines:
                            _flush()
                            continue
                        max_units = carton
                    take = min(remain, max_units)
                    take = (take // carton) * carton
                    if take <= 0:
                        take = min(remain, carton)
                    if take <= 0:
                        break

                    piece = dict(ln)
                    piece['pack_qty'] = int(take)
                    piece['line_volume_m3'] = round(take * unit_vol, 3)
                    current_lines.append(piece)
                    used_vol += take * unit_vol
                    remain -= take

                    if used_vol >= vol_cap * min_fill:
                        _flush()
    ...
    def handle_container_draft_api(self, environ, method, start_response):
        """GET：目的区域列表；POST：生成按工厂分组的凑柜草稿。"""
        try:
            if method == 'GET':
                with self._get_db_connection() as conn:
                    destination_regions = self._forecast_list_destination_regions(conn)
            ...
            data = self._read_json_body(environ) or {}
            months = self._forecast_parse_months(data.get('months') or [])
            if not months:
                months = self._forecast_default_future_months(6)

            hist_start_raw = str(data.get('history_start') or '').strip()
            hist_end_raw = str(data.get('history_end') or '').strip()
            hist_start, hist_end = self._forecast_history_month_range(hist_start_raw, hist_end_raw, default_months=12)
            history_months = self._forecast_iter_months(hist_start, hist_end)

            lead_time_region = str(data.get('lead_time_region') or '').strip()
            lead_times = data.get('lead_times') if isinstance(data.get('lead_times'), dict) else {}
            target_cover = max(0.0, float(data.get('target_cover_months') or 2))
            container_volume = max(1.0, float(data.get('container_volume_m3') or 69.0))
            min_fill_ratio = float(data.get('min_fill_ratio') or 0.75)
            only_shortage = str(data.get('only_shortage', True)).lower() not in ('0', 'false', 'no', 'off')

            sf_shop_ids = []
            raw_shops = data.get('shop_ids')
            if isinstance(raw_shops, list):
                sf_shop_ids = [self._parse_int(x) for x in raw_shops if self._parse_int(x)]

            query_params = {}
            for k in ('sku', 'spec', 'fabric'):
            ...
            with self._get_db_connection() as conn:
                destination_regions = self._forecast_list_destination_regions(conn)
                perf_max = self._forecast_perf_max_record_date(conn)
                cur_key = self._forecast_current_month_key()
                mtd_in = cur_key in set(history_months or [])
                mtd_ctx = self._forecast_mtd_time_context(perf_max, cur_key) if mtd_in else None

                rows = self._forecast_build_order_rows(
                    conn, query_params, months, hist_start, hist_end, sf_group=None, shop_ids=sf_shop_ids or None
                )
                self._forecast_attach_inventory_to_rows(
                    conn, rows, 'order', history_months,
                    hist_start=hist_start, hist_end=hist_end, shop_ids=sf_shop_ids or None,
                )
                self._forecast_attach_surplus_detail_to_order_rows(conn, rows)
                ...
                for row in rows:
                    ...
                    if oid:
                        op_ids.append(oid)
                pack_meta = self._container_load_pack_meta_by_order(conn, op_ids)
            ...
            for row in rows:
                ...
                if not oid:
                    continue
                surplus = self._container_compute_full_surplus_series(
                    row, months, lead_time_region, lead_times, mtd_ctx=mtd_ctx
                )
                raw_qty, sug_meta = self._container_suggest_ship_qty(row, months, surplus, target_cover)
                meta = pack_meta.get(oid) or {}
                carton = meta.get('carton_qty') or meta.get('carton_qty_calc') or 1
                suggested = self._container_round_up_cartons(raw_qty, carton)

                if only_shortage and suggested <= 0:
                    continue
                if suggested <= 0:
                    skipped.append({
                        'order_product_id': oid,
                        'sku': labels.get('sku') or meta.get('sku') or '',
                        'reason': '预测期内无缺口',
                    })
                    continue

                unit_vol = meta.get('unit_volume_m3')
                if not unit_vol and carton:
                    unit_vol = container_volume / float(max(1, int(carton)))

                pack_lines.append({
                    'order_product_id': oid,
                    'sku': labels.get('sku') or meta.get('sku') or '',
                    'sku_family': labels.get('sku_family') or '',
                    'factory_id': int(meta.get('factory_id') or 0),
                    'factory_name': meta.get('factory_name') or '未关联工厂',
                    'suggested_qty': int(suggested),
                    'raw_suggested_qty': int(raw_qty),
                    'carton_qty': int(carton),
                    'unit_volume_m3': round(float(unit_vol or 0), 6) if unit_vol else None,
                    'first_shortage_month': sug_meta.get('first_shortage_month'),
                    'min_full_surplus': sug_meta.get('min_full_surplus'),
                    'end_full_surplus': sug_meta.get('end_full_surplus'),
                    'avg_monthly_demand': sug_meta.get('avg_monthly_demand'),
                })

            containers = self._container_pack_lines_by_factory(
                pack_lines, container_volume, min_fill_ratio=min_fill_ratio
            )

            unassigned = [ln for ln in pack_lines if int(ln.get('factory_id') or 0) == 0]
            summary = {
                'sku_count': len(pack_lines),
                'container_count': len(containers),
                'total_units': sum(int(c.get('total_units') or 0) for c in containers),
                'unassigned_factory_skus': len(unassigned),
            }
            ...
            return self.send_json({
                ...
                'params': {
                    'months': months,
                    'history_range': {'start': hist_start, 'end': hist_end},
                    'lead_time_region': lead_time_region,
                ...
                'unpacked_candidates': pack_lines,
                'skipped': skipped,
            }, start_response)
        except Exception as e:
            return self.send_json({'status': 'error', 'message': str(e)}, start_response)
