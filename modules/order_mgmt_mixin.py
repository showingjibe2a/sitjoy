...
class OrderManagementMixin:
    ...
    def _off_market_count_pending_variant_links(self, conn, owner_id):
        """sales_variant_order_links 中仍绑定本下单 SKU 的规格行数（与单笔迁移一致）。"""
        oid = self._parse_int(owner_id)
        if not oid:
            return 0
        with conn.cursor() as cur:
    ...
    def _handle_off_market_ship_plan_batch_preview(self, environ, start_response, query_params):
        """批量核验：已下市 SKU 是否存在「全部在市」替代发货方案及待迁移 sales_variant_order_links 行数。"""
