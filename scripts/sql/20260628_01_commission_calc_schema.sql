...
INSERT INTO `commission_calc_rules` (`platform_type_id`, `commission_group`, `calc_method`, `params_json`, `label`)
SELECT pt.`id`, '家具', 'tiered', CAST('{"tiers":[{"up_to":200,"rate":0.15},{"rate":0.10}]}' AS JSON), '家具分段15%/10%'
...
-- Wayfair 全品类固定 4%
INSERT INTO `commission_calc_rules` (`platform_type_id`, `commission_group`, `calc_method`, `params_json`, `label`)
SELECT pt.`id`, '全品类', 'flat', CAST('{"rate":0.04}' AS JSON), '全品类固定4%'
FROM `platform_types` pt
